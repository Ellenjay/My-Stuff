...
Start  	CLRF	NumH
		CLRF	NumL 
		Call 	InitDeltaSigA2D ; Initialize 
        Call	DeltaSigA2D		; Read Variable resistor into 16-bit binary number in NumH:NumL 	
        
        movfw	NumH		; copy variable resitor setting
        movwf	UserH		; to User High
        Movfw	NumL		; and
        movwf	UserL		; User Low. Will be used for comparing Set temp to actual temp later in program
        
        Call 	Convert			; Convert above reading into BCD equivalent in TenK:Thou:Hund:Tens:Ones.
		...
		movfw	NumH		; copy probe reading
        movwf	ProbeH		; to Probe High
        Movfw	NumL		; and
        movwf	ProbeL		; Probe Low. Will be used for comparing Set temp to actual temp later in program
		...
		Call	Display	
  		...
  		Call Compare ;Compare Probe and user setting
 		skpc	;skip next instruction if User setting is higher than Probe
 		BSF		PORTB,0
		skpnc	;skip next instruction if User setting is = or lower than Probe
		BCF		PORTB,0	
		nop
		Goto	Start
...
Compare 	movf UserH,w
			subwf ProbeH,w ; subtract Y-X
			Are_they_equal:
			; Are they equal ?
			skpz
			goto results16
...
BitLoop		Call GetNextBit
			...
			BTFSC	PORTB,2
			BSF		NumL,0
			; Bit D2 would be high if the thermocouple is open.
