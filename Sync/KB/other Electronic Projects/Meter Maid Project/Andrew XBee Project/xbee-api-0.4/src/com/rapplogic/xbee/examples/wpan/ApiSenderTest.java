	...
	private ApiSenderTest() throws Exception {
		...
		try {
			...
			while (true) {
				...
				XBeeAddress16 destination = new XBeeAddress16(0x50, 0x01);
				
				TxRequest16 tx = new TxRequest16(destination, frameId, payload);
				log.debug("Payload =  " + payload.toString());//sam
				...
				while (true) {
					...
					log.debug("expected tx status but received " + response.toString());//sam
